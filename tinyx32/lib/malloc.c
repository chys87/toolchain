...
static_assert((sizeof(AllocedBlock) & (sizeof(AllocedBlock) - 1)) == 0, "sizeof(AllocedBlock)");
...
void *malloc(size_t n) {
    return aligned_alloc(sizeof(AllocedBlock), n);
}
...
void *aligned_alloc(size_t align, size_t n) {
    if ((align == 0) || (align & (align - 1)) != 0 || n == 0)
        return NULL;
    if (align < sizeof(AllocedBlock))
        align = sizeof(AllocedBlock);
    ...
    for (FreeBlock *p = free_block_chain; p; prev = &p->next, p = p->next) {
        if (((uintptr_t)p & (align - 1)) == 0 && p->size >= total_size) {
            AllocedBlock *alloced = (AllocedBlock *)((char *)p + align - sizeof(AllocedBlock));
    ...
    void *np = fsys_mmap(NULL, mmap_size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    if (fsys_mmap_failed(np))
        return NULL;
    ...
    AllocedBlock *alloced = (AllocedBlock *)((char *)np + align - sizeof(AllocedBlock));
    alloced->align = align;
    alloced->size = total_size;
