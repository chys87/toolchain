...
const Browser supported_browsers[] = {
    {STR_LEN("Google Chrome"), STR_LEN("/chrome"), {"google-chrome-stable", "google-chrome"}},
    {STR_LEN("Firefox"), STR_LEN("/firefox-bin"), {"firefox", "firefox-bin"}},
};

constexpr unsigned kBrowsers = sizeof(supported_browsers) / sizeof(supported_browsers[0]);

const Browser *identify_browser(std::string_view exe) {
    for (const Browser &browser: supported_browsers)
        if (browser.key_len <= exe.length() && memcmp(browser.key, exe.data() + exe.length() - browser.key_len, browser.key_len) == 0)
            return &browser;
    return nullptr;
}
    ...
    while ((l = fsys_getdents64(fd_proc, buf, sizeof(buf))) > 0) {
        ...
        do {
            ...
            if (!(exe_l > 0 && exe_l < (ssize_t)sizeof(exe_full_buf)))
                continue;
            if (const Browser *new_match = identify_browser({exe_full_buf, size_t(exe_l)})) {
...
int running_browser_main(size_t argc, char **argv, char **envp) {
    ...
    if (const Browser *browser = find_running_browser()) {
        ...
        cm << pair{browser->name, size_t(browser->name_len)} << " seems to be running, but we're unable to run it.\n";
